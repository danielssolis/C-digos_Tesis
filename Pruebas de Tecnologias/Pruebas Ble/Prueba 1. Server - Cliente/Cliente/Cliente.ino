...
bool connectToServer() {
  ...
  if (!pChar->canWrite()) {
    Serial.println("❌ La característica no es writable.");
    pClient->disconnect();
    return false;
  }
...
void loop() {
  if (pClient && pClient->isConnected() && pChar) {
    // Simular datos (temperatura y humedad)
    float temp = random(200, 300) / 10.0;  // 20.0 a 30.0 °C
    float hum = random(400, 700) / 10.0;   // 40.0 a 70.0 %

    String data = "Temp: " + String(temp, 1) + " °C, Hum: " + String(hum, 1) + " %";
